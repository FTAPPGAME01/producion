...
let gameState = {
    ...
    takenCount: 0,
    timeLeft: 10,
    currentGameId: Date.now(), // Identificador único para la partida actual
    lastReset: Date.now()
};
// Mantener un registro de la última partida válida
let lastValidGameState = null;
io.on('connection', (socket) => {
    console.log('Nuevo cliente conectado');

    // Cuando un nuevo cliente se conecta, enviar el estado actual
    socket.emit('gameState', {
        state: lastValidGameState || gameState,
        gameId: gameState.currentGameId
    });

    // Manejar la sincronización inicial del cliente
    ...
    socket.on('updateState', (updatedState) => {
        // Verificar si el estado actualizado corresponde a la partida actual
        if (updatedState.gameId === gameState.currentGameId) {
            lastValidGameState = {...updatedState};
            // Emitir el estado actualizado a todos los clientes excepto al emisor
            socket.broadcast.emit('stateChanged', lastValidGameState);
        }
    });

    // Cuando se toma una ficha
    socket.on('takeToken', (data) => {
        if (lastValidGameState) {
            // Actualizar el estado de la ficha en el estado válido
            const { rowId, index, player } = data;
            lastValidGameState[rowId][index].available = false;
            lastValidGameState[rowId][index].takenBy = player;
            lastValidGameState.takenCount++;

            // Emitir el estado actualizado a todos
            io.emit('stateChanged', lastValidGameState);
        }
    });
});
